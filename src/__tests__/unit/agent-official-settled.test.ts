...
describe('official settled strategy positions', () => {
  it('allocates only attributed remaining lots to an official settlement', () => {
    const rows = buildOfficialSettledStrategyPositions(
      [{ id: 's1', platform: 'kalshi', marketId: 'KX1', marketResult: 'YES', yesQuantity: '5', noQuantity: '0', yesCost: '2', noCost: '0', revenue: '5', fee: '0', settledAt: '2026-07-13T02:00:00Z' }],
    ...
    expect(rows).toMatchObject([{ shares: 5, avg_price: 0.4, settlement_price: 1, cost_basis: 2, proceeds: 5, realized_pnl: 3 }]);
  });
});
