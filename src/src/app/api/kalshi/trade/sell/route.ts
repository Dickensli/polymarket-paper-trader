...
const kalshiSellSchema = z.object({
...
export async function POST(request: NextRequest) {
  try {
    ...
    const parsed = kalshiSellSchema.safeParse(await request.json());
    if (!parsed.success) {
    ...
    const position = order.positionId
      ? await db.query.positions.findFirst({
          where: and(eq(positions.id, order.positionId), eq(positions.userId, session.user.id), eq(positions.isOpen, true)),
        })
      : order.ticker
        ? await db.query.positions.findFirst({
            where: and(
              eq(positions.userId, session.user.id),
              eq(positions.marketId, order.ticker),
    ...
    if (!position) {
      return NextResponse.json({ error: 'Kalshi position not found' }, { status: 404 });
    }
    ...
    const shares = order.quantity === 'ALL' ? heldShares : Math.min(order.quantity, heldShares);
    const trade = await executeTrade(session.user.id, {
      marketId: ticker,
      marketQuestion: position.marketQuestion || ticker,
      tokenId: kalshiTokenId(ticker, outcome),
      outcome,
      side: 'SELL',
      shares,
