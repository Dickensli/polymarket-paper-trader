  ...
  try {
    ...
    const currentPortfolios = filteredStrategies.flatMap((strategy) => {
      ...
      if (strategy.agentMode !== 'paper') {
        return [{
          ...
          pnl: numeric(latestSnapshot.totalValue) - numeric(strategy.startingBalance || 0),
    ...
    const payload = {
      ...
      snapshots: filteredSnapshots.map((snapshot) => ({
        ...
        pnl: numeric(snapshot.totalValue) - numeric(strategyById.get(snapshot.strategyId || '')?.startingBalance || 0),
