    ...
    if (res.ok) {
        ...
        if (searchRes.ok) {
            ...
            if (events[0] && events[0].markets) {
                market = events[0].markets.find((m) => m.slug === slugOrId || m.id === slugOrId);
            }
        }
    }
    if (!market) {
        ...
        switch (name) {
            ...
            case "portfolio": {
                ...
                const formatted = {
                    ...
                    recent_trades: (portfolio.tradeHistory || []).slice(0, 10).map((t) => ({
                        side: t.side,
                        outcome: t.outcome,
            ...
            case "history": {
                ...
                const data = await res.json();
                const portfolio = data.data || data;
                const trades = (portfolio.tradeHistory || []).slice(0, limit).map((t) => ({
                    trade_id: t.id,
                    market: t.marketQuestion,
                    outcome: t.outcome,
                    side: t.side,
            ...
            case "stats": {
                ...
                const winTrades = trades.filter((t) => t.side === "SELL" && parseFloat(t.total) > 0); // basic win stats
                return {
                    content: [{
                            type: "text",
                            text: JSON.stringify({
