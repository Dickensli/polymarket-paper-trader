            ...
            // ── Account / Lifecycle ──────────────────────────────────────────
            {
                ...
                inputSchema: {
                    ...
                    properties: {
                        ...
                        account_id: { type: "string", description: "Optional override for the agent user ID (UUID)" }
                    },
                    required: ["account"]
                }
            },
            ...
            },
            {
                ...
                inputSchema: {
                    ...
                    properties: {
                        ...
                        account_id: { type: "string", description: "Optional override for the agent user ID" },
                    },
                    required: ["trades"]
                }
            },
    ...
    try {
        switch (name) {
            ...
            case "backtest": {
                ...
                const startingBalance = args?.balance || 10000;
            ...
            // ── Agent Reports ──────────────────────────────────────────
            case "save_report": {
                const strategy_id = (args?.strategy_id || args?.strategy_name || args?.account);
            ...
            case "list_reports": {
                ...
                const listRes = await fetch(`${POLYTRADER_API_URL}/agent/reports?strategy_id=${encodeURIComponent(strategy_id)}&limit=${limit}`, { headers: getAgentHeaders(args) });
                if (!listRes.ok) {
            ...
            case "read_report": {
                ...
                if (!strategy_id || !filename)
                    throw new Error("Missing required fields: strategy_id, filename");
                const readRes = await fetch(`${POLYTRADER_API_URL}/agent/reports?strategy_id=${encodeURIComponent(strategy_id)}&filename=${encodeURIComponent(filename)}`, { headers: getAgentHeaders(args) });
                if (!readRes.ok) {
            ...
            case "register_strategy": {
                const strategy_id = (args?.strategy_id || args?.strategy_name);
                if (!strategy_id)
                    throw new Error("Missing required field: strategy_id or strategy_name");
                const account_id = (args?.account_id || args?.agent_user_id || args?.account);
                if (!account_id)
                    throw new Error("Missing required field: account_id or agent_user_id");
            ...
            case "get_strategy_context": {
                ...
                if (!strategy_id)
                    throw new Error("Missing required field: strategy_id or strategy_name");
                const res = await fetch(`${POLYTRADER_API_URL}/agent/context?strategy_id=${encodeURIComponent(strategy_id)}`, { headers: getAgentHeaders(args) });
                if (!res.ok) {
                    const errBody = await res.text();
                    throw new Error(`API returned ${res.status}: ${errBody}`);
                }
