...
1. **STRATEGY ID LOCK**: All trading agents MUST use their exact assigned trigger ID (e.g. `high_freq_retro`, `aggressive_retro_real`) for all tool calls. You are STRICTLY forbidden from dynamically calling `register_strategy` with custom, invented, or modified strategy IDs (e.g. adding `_sol` or `_v2` suffixes). The MCP servers enforce this at runtime.
