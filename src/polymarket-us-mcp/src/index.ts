...
function positionRecord(position: Record<string, unknown>) {
  ...
  return {
    ...
    outcome: outcome === "NO" ? "NO" as const : "YES" as const,
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  switch (name) {
    ...
    case "get_market_book": {
      const data = await callPolyTrader(`/polymarket-us/markets/${encodeURIComponent(String((args as any).slug))}/book?outcome=${encodeURIComponent(String((args as any).outcome))}`, {
        headers: getPublicHeaders(),
      });
      return json({ ok: true, data: data.data ?? data });
    }
    ...
    case "sell": {
      ...
      if (shares === undefined || !slug || sellArgs.positionId) {
        ...
        const position = positions.find((candidate) => {
          ...
          if (sellArgs.positionId) return resolved.id === String(sellArgs.positionId);
    ...
    case "list_reports": {
      const strategy_id = String((args as any).strategy_id);
      if (!strategy_id) throw new Error("Missing required field: account");
    ...
    case "read_report": {
      const strategy_id = String((args as any).strategy_id);
      const filename = String((args as any).filename);
      if (!strategy_id || !filename) throw new Error("Missing required fields: account, filename");
      const data = await callPolyTrader(
